...
#include "tiny_ynn/util/random.h"
...
class CharDataset {
 public:
  ...
  const bool get() {
    ...
    while (count < batch_size_ && fdata_.read(&x, 1) && flabels_.read(&y, 1)) {
      tiny_ynn::fill_tensor(data_buffer_[count], 0.0);
      tiny_ynn::fill_tensor(labels_buffer_[count], 0.0);
   ...
   * @return data_buffer
   */
  std::vector<tiny_ynn::tensor_t> &get_data() { return data_buffer_; }
  /**
   * Get labels buffer.
   * @return labels_buffer
   */
  std::vector<tiny_ynn::tensor_t> &get_labels() { return labels_buffer_; }
 ...
 private:
  ...
  // buffers
  std::vector<tiny_ynn::tensor_t> data_buffer_;
  std::vector<tiny_ynn::tensor_t> labels_buffer_;
};
...
void construct_net(N &nn,
  ...
  using activation = tiny_ynn::selu_layer;
  ...
  // clip gradients
  tiny_ynn::recurrent_layer_parameters params;
  params.clip = 0;
  ...
  for (int i = 0; i < n_layers; i++) {
    if (rnn_type == "rnn") {
      nn << recurrent(tiny_ynn::rnn(input_size, hidden_size), seq_len, params);
    } else if (rnn_type == "gru") {
      nn << recurrent(tiny_ynn::gru(input_size, hidden_size), seq_len, params);
    } else if (rnn_type == "lstm") {
      nn << recurrent(tiny_ynn::lstm(input_size, hidden_size), seq_len, params);
    }
 ...
 */
const float cross_entropy(std::vector<tiny_ynn::tensor_t> data,
                          std::vector<tiny_ynn::tensor_t> labels) {
  float loss = 0;
 ...
 * @return sample.
 */
unsigned int select_one(const tiny_ynn::vec_t probs) {
...
void set_train(N &nn, const int seq_len) {
  nn.set_netphase(tiny_ynn::net_phase::train);
  for (unsigned int i = 0; i < nn.layer_size(); i++) {
    try {
      nn.template at<tiny_ynn::dropout_layer>(i).set_context(
        tiny_ynn::net_phase::train);
    } catch (tiny_ynn::nn_error &err) {
    }
    try {
      nn.template at<tiny_ynn::recurrent_layer>(i).seq_len(seq_len);
      nn.template at<tiny_ynn::recurrent_layer>(i).bptt_max(seq_len);
      nn.template at<tiny_ynn::recurrent_layer>(i).clear_state();
    } catch (tiny_ynn::nn_error &err) {
    }
  }
}
...
template <typename N>
void set_test(N &nn) {
  nn.set_netphase(tiny_ynn::net_phase::test);
  for (unsigned int i = 0; i < nn.layer_size(); i++) {
    try {
      nn.template at<tiny_ynn::dropout_layer>(i).set_context(
        tiny_ynn::net_phase::test);
    } catch (tiny_ynn::nn_error &err) {
    }
    try {
      nn.template at<tiny_ynn::recurrent_layer>(i).clear_state();
...
void train(int hidden_size,
  ...
  nn.weight_init(tiny_ynn::weight_init::xavier());
  ...
  std::vector<tiny_ynn::tensor_t> cost;
  while (epoch++ < n_train_epochs) {
    ...
    while (train_dataset.get()) {
      std::vector<tiny_ynn::tensor_t> &data   = train_dataset.get_data();
      std::vector<tiny_ynn::tensor_t> &labels = train_dataset.get_labels();
      data                                    = nn.fprop(data);
      ...
      nn.bprop<tiny_ynn::cross_entropy>(data, labels, cost);
    ...
    while (val_dataset.get()) {
      std::vector<tiny_ynn::tensor_t> &data   = val_dataset.get_data();
      std::vector<tiny_ynn::tensor_t> &labels = val_dataset.get_labels();
      data                                    = nn.fprop(data);
...
static tiny_ynn::core::backend_t parse_backend_name(const std::string &name) {
  const std::array<const std::string, 5> names = {
    "internal", "nnpack", "libdnn", "avx", "opencl",
  };
  for (unsigned int i = 0; i < names.size(); ++i) {
    if (name.compare(names[i]) == 0) {
      return static_cast<tiny_ynn::core::backend_t>(i);
    }
  }
  return tiny_ynn::core::default_engine();
}
...
int main(int argc, char **argv) {
  ...
  tiny_ynn::core::backend_t backend_type = tiny_ynn::core::default_engine();
