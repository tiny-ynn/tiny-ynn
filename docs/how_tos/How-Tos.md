# How-Tos
Details about tiny-ynn's API and short examples.
...
If you feel these syntax a bit redundant, you can also use "shortcut" names defined in tiny_ynn.h.
```cpp
...
> Note: ```operator <<``` and ```operator >>``` APIs before tiny-ynn v0.1.1 are deprecated.

### import caffe's model
[Import Caffe Model to tiny-ynn](../examples/caffe_converter/readme.md)
...
### reading images
You can use a simple ```tiny_ynn::image``` class to handle your images. JPEG (baseline & progressive), PNG (1/2/4/8 bit per channel), BMP (non-1bp, non-RLE), GIF are supported reading formats. Note that it's memory layout differs from OpenCV - it's layout is KHW (K:channels, H:height, W:width).
...
//      R2, R3]       G2, G3]       B2, B3]
//
// memory layout of tiny_ynn::image is KHW, and order of channels K depends on its image_type:
//
...
// convert into tiny-ynn's interface type
...
In neural network training, initial value of weight/bias can affect training speed and accuracy. In tiny-ynn, the weight is appropriately scaled by xavier algorithm[1](http://jmlr.org/proceedings/papers/v9/glorot10a/glorot10a.pdf) and the bias is filled with 0.
...
## handle errors
When some error occurs, tiny-ynn doesn't print any message on stdout. Instead of ```printf```, tiny-ynn throws exception.
This behaviour is suitable when you integrate tiny-ynn into your application (especially embedded systems).

### catch application exceptions
tiny-ynn may throw one of the following types:

- ```tiny_ynn::nn_error```
- ```tiny_ynn::not_implemented_error```
- ```std::bad_alloc```
