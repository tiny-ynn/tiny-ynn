...
/**
 * A model of neural networks in tiny-ynn
 *
...
class network {
 public:
...
#else
    throw nn_error("tiny-ynn was not built with Serialization support");
  ...
  void save(std::ostream &os) const {
    os.precision(std::numeric_limits<tiny_ynn::float_t>::digits10);
    net_.save(os);
  }

  ///< @deprecated use load(filename,target,format) instead.
  void load(std::istream &is) {
    is.precision(std::numeric_limits<tiny_ynn::float_t>::digits10);
    net_.load(is);
  }
