...
class alexnet : public tiny_ynn::network<tiny_ynn::sequential> {
 public:
  explicit alexnet(const std::string &name = "")
    : tiny_ynn::network<tiny_ynn::sequential>(name) {
    // todo: (karandesai) shift this to tiny_ynn::activation
    using relu     = tiny_ynn::activation::relu;
    using conv     = tiny_ynn::layers::conv;
    using max_pool = tiny_ynn::layers::max_pool;
