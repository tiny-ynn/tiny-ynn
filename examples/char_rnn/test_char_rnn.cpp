...
// Data helpers
std::vector<tiny_ynn::tensor_t> encode(char &input, std::map<char, int> &dict) {
  std::vector<tiny_ynn::tensor_t> ret(1);
  ret[0].resize(1);
 ...
 */
void softmax(tiny_ynn::vec_t &data, double temperature = 1.0) {
  tiny_ynn::softmax_layer softmax;
  for (auto &d : data) {
    d /= temperature;
  }
...
void test(int hidden_size,
          ...
          const std::string rnn_type,
          tiny_ynn::core::backend_t backend_type) {
  using recurrent_layer = tiny_ynn::recurrent_layer;
