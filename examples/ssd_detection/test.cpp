...
void convert_image(const std::string& imagefilename,
  ...
  tiny_ynn::image<> img(imagefilename, tiny_ynn::image_type::bgr);
...
void concat_hwc_features(tiny_ynn::vec_t& collections,
                         size_t n_collection_items,
                         tiny_ynn::vec_t& feature,
                         size_t in_spatial_size,
                         size_t in_channels) {
  tiny_ynn::vec_t t_feature;
...
void inline_softmax(tiny_ynn::vec_t& confidences) {
...
void save_default_boxes(tiny_ynn::vec_t& default_boxes,
                        size_t box_index,
                        float cx,
                        float cy,
                        float width,
...
void init_default_boxes(tiny_ynn::vec_t& default_boxes) {
...
void construct_nets(std::vector<tiny_ynn::network<tiny_ynn::sequential>>& nets,
                    const std::string& modelFolder) {
  using conv   = tiny_ynn::convolutional_layer;
  using pool   = tiny_ynn::max_pooling_layer;
  using relu   = tiny_ynn::relu_layer;
  using l2norm = tiny_ynn::l2_normalization_layer;
  using pad    = tiny_ynn::zero_pad_layer;

  tiny_ynn::network<tiny_ynn::sequential> nn1;
  nn1 << conv(300, 300, 3, 3, 64, tiny_ynn::padding::same)             // vgg.0
      << relu() << conv(300, 300, 3, 64, 64, tiny_ynn::padding::same)  // vgg.2
      << relu() << pool(300, 300, 64, 2, 2, false)
      << conv(150, 150, 3, 64, 128, tiny_ynn::padding::same)  // vgg.5
      << relu()
      << conv(150, 150, 3, 128, 128, tiny_ynn::padding::same)  // vgg.7
      << relu() << pool(150, 150, 128, 2, 2, false)
      << conv(75, 75, 3, 128, 256, tiny_ynn::padding::same)            // vgg.10
      << relu() << conv(75, 75, 3, 256, 256, tiny_ynn::padding::same)  // vgg.12
      << relu() << conv(75, 75, 3, 256, 256, tiny_ynn::padding::same)  // vgg.14
      << relu() << pool(75, 75, 256, 2, 2, true)
      << conv(38, 38, 3, 256, 512, tiny_ynn::padding::same)            // vgg.17
      << relu() << conv(38, 38, 3, 512, 512, tiny_ynn::padding::same)  // vgg.19
      << relu() << conv(38, 38, 3, 512, 512, tiny_ynn::padding::same)  // vgg.21
      << relu() << l2norm(38 * 38, 512, 1e-12, 20);
  nets.push_back(nn1);

  tiny_ynn::network<tiny_ynn::sequential> nn2;
  nn2 << pool(38, 38, 512, 2, 2, false)
      << conv(19, 19, 3, 512, 512, tiny_ynn::padding::same)            // vgg.24
      << relu() << conv(19, 19, 3, 512, 512, tiny_ynn::padding::same)  // vgg.26
      << relu() << conv(19, 19, 3, 512, 512, tiny_ynn::padding::same)  // vgg.28
      << relu() << pad(19, 19, 512, 1, 1) << pool(21, 21, 512, 3, 1, true)
      << pad(19, 19, 512, 6, 6)
      << conv(31, 31, 3, 512, 1024, tiny_ynn::padding::valid, true, 1, 1, 6,
              6)  // vgg.31
      << relu()
      << conv(19, 19, 1, 1024, 1024, tiny_ynn::padding::same)  // vgg.33
      << relu();
  nets.push_back(nn2);

  tiny_ynn::network<tiny_ynn::sequential> nn3;
  nn3 << conv(19, 19, 1, 1024, 256, tiny_ynn::padding::same)  // extra.0
      << relu() << pad(19, 19, 256, 1, 1)
      << conv(21, 21, 3, 256, 512, tiny_ynn::padding::valid, true, 2,
              2)  // extra.1
      << relu();
  nets.push_back(nn3);

  tiny_ynn::network<tiny_ynn::sequential> nn4;
  nn4 << conv(10, 10, 1, 512, 128, tiny_ynn::padding::same)  // extra.2
      << relu() << pad(10, 10, 128, 1, 1)
      << conv(12, 12, 3, 128, 256, tiny_ynn::padding::valid, true, 2,
              2)  // extra.3
      << relu();
  nets.push_back(nn4);

  tiny_ynn::network<tiny_ynn::sequential> nn5;
  nn5 << conv(5, 5, 1, 256, 128, tiny_ynn::padding::same)             // extra.4
      << relu() << conv(5, 5, 3, 128, 256, tiny_ynn::padding::valid)  // extra.5
      << relu();
  nets.push_back(nn5);

  tiny_ynn::network<tiny_ynn::sequential> nn6;
  nn6 << conv(3, 3, 1, 256, 128, tiny_ynn::padding::same)             // extra.6
      << relu() << conv(3, 3, 3, 128, 256, tiny_ynn::padding::valid)  // extra.7
      << relu();
  nets.push_back(nn6);

  // Locations
  tiny_ynn::network<tiny_ynn::sequential> nn7;
  nn7 << conv(38, 38, 3, 512, 16, tiny_ynn::padding::same);  // loc.0
  nets.push_back(nn7);

  tiny_ynn::network<tiny_ynn::sequential> nn8;
  nn8 << conv(19, 19, 3, 1024, 24, tiny_ynn::padding::same);  // loc.1
  nets.push_back(nn8);

  tiny_ynn::network<tiny_ynn::sequential> nn9;
  nn9 << conv(10, 10, 3, 512, 24, tiny_ynn::padding::same);  // loc.2
  nets.push_back(nn9);

  tiny_ynn::network<tiny_ynn::sequential> nn10;
  nn10 << conv(5, 5, 3, 256, 24, tiny_ynn::padding::same);  // loc.3
  nets.push_back(nn10);

  tiny_ynn::network<tiny_ynn::sequential> nn11;
  nn11 << conv(3, 3, 3, 256, 16, tiny_ynn::padding::same);  // loc.4
  nets.push_back(nn11);

  tiny_ynn::network<tiny_ynn::sequential> nn12;
  nn12 << conv(1, 1, 3, 256, 16, tiny_ynn::padding::same);  // loc.5
  nets.push_back(nn12);

  // Confidences
  tiny_ynn::network<tiny_ynn::sequential> nn13;
  nn13 << conv(38, 38, 3, 512, 4 * N_CLASSES,
               tiny_ynn::padding::same);  // conf.0
  nets.push_back(nn13);

  tiny_ynn::network<tiny_ynn::sequential> nn14;
  nn14 << conv(19, 19, 3, 1024, 6 * N_CLASSES,
               tiny_ynn::padding::same);  // conf.1
  nets.push_back(nn14);

  tiny_ynn::network<tiny_ynn::sequential> nn15;
  nn15 << conv(10, 10, 3, 512, 6 * N_CLASSES,
               tiny_ynn::padding::same);  // conf.2
  nets.push_back(nn15);

  tiny_ynn::network<tiny_ynn::sequential> nn16;
  nn16 << conv(5, 5, 3, 256, 6 * N_CLASSES, tiny_ynn::padding::same);  // conf.3
  nets.push_back(nn16);

  tiny_ynn::network<tiny_ynn::sequential> nn17;
  nn17 << conv(3, 3, 3, 256, 4 * N_CLASSES, tiny_ynn::padding::same);  // conf.4
  nets.push_back(nn17);

  tiny_ynn::network<tiny_ynn::sequential> nn18;
  nn18 << conv(1, 1, 3, 256, 4 * N_CLASSES, tiny_ynn::padding::same);  // conf.5
  nets.push_back(nn18);
...
void detect(std::vector<tiny_ynn::network<tiny_ynn::sequential>>& nets,
  ...
  std::vector<tiny_ynn::bounding_box> bounding_box_candidates;
  for (size_t i = 0; i < bounding_box_indexes.size(); ++i) {
    int bbox_index = bounding_box_indexes[i];

    tiny_ynn::bounding_box bbox;
  ...
  bounding_box_indexes = tiny_ynn::nms(bounding_box_candidates, NMS_THRESHOLD);
  ...
  for (size_t i = 0; i < bounding_box_indexes.size(); ++i) {
    int bbox_index              = bounding_box_indexes[i];
    tiny_ynn::bounding_box bbox = bounding_box_candidates[bbox_index];
...
int main(int argc, char** argv) {
  ...
  std::vector<tiny_ynn::network<tiny_ynn::sequential>> nets;
