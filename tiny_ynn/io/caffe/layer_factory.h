...
#include "tiny_ynn/io/caffe/caffe.pb.h"

#include "tiny_ynn/io/caffe/layer_factory_impl.h"
...
inline std::shared_ptr<network<sequential>> create_net_from_caffe_net(
  ...
  for (size_t i = 0; i < src_net.size(); i++) {
    ...
    if (!detail::layer_supported(type)) {
      throw nn_error("error: tiny-ynn does not support this layer type:" +
                     type);
    }
 ...
 * @param net [out] tiny-ynn's network
 */
template <typename N>
inline void reload_weight_from_caffe_net(const caffe::NetParameter &layer,
  ...
  for (size_t caffe_layer_idx = 0; caffe_layer_idx < src_net.size();
    ...
    if (tiny_layer_idx >= net->depth()) {
      throw nn_error(
        "error: tiny-ynn does not support loading weights "
        "for this layer type: " +
        type);
    }
