...
#include "tiny_ynn/util/util.h"

namespace cereal {

template <typename Archive>
void save(Archive &ar, const std::vector<tiny_ynn::layer *> &v) {
#ifndef CNN_NO_SERIALIZATION
  ar(cereal::make_size_tag(static_cast<cereal::size_type>(v.size())));
  for (auto n : v) {
    tiny_ynn::layer::save_layer(ar, *n);
  }
#else
  throw tiny_ynn::nn_error("tiny-ynn was not built with Serialization support");
#endif  // CNN_NO_SERIALIZATION
}

template <typename Archive>
void load(Archive &ar, std::vector<std::shared_ptr<tiny_ynn::layer>> &v) {
#ifndef CNN_NO_SERIALIZATION
  cereal::size_type size;
  ar(cereal::make_size_tag(size));

  for (size_t i = 0; i < size; i++) {
    v.emplace_back(tiny_ynn::layer::load_layer(ar));
  }
#else
  throw tiny_ynn::nn_error("tiny-ynn was not built with Serialization support");
#endif  // CNN_NO_SERIALIZATION
 ...
 * Node is a computational unit of tiny-ynn (for example, convolution).
 ...
 * given node alive. If given type is lvalue, tiny-ynn holds raw-pointer only
