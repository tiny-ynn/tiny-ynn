...
class Model {
 public:
  ...
  Model(const char *weights,
    ...
    for (int i = 0; i < depth; i++) {
      if (rnn_type_ == "rnn") {
        nn << recurrent(tiny_ynn::rnn(input_size, hidden_size), 1, params);
      } else if (rnn_type_ == "gru") {
        nn << recurrent(tiny_ynn::gru(input_size, hidden_size), 1, params);
      } else if (rnn_type_ == "lstm") {
        nn << recurrent(tiny_ynn::lstm(input_size, hidden_size), 1, params);
      }
 ...
 private:
   ...
   * @return encoded char.
   */
  std::vector<tiny_ynn::tensor_t> encode(char &input,
