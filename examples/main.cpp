...
void sample1_convnet(const std::string& data_dir) {
  ...
  tiny_ynn::network<sequential> nn;
...
void sample2_mlp(const string& data_dir) {
  ...
  auto on_enumerate_epoch = [&]() {
    ...
    optimizer.alpha = std::max((tiny_ynn::float_t)0.00001, optimizer.alpha);
...
#include "tiny_ynn/util/target_cost.h"
...
void sample5_unbalanced_training_data(const string& data_dir) {
  ...
  auto on_enumerate_epoch = [&]() {
    ...
    optimizer.alpha *= 0.85;  // decay learning rate
    optimizer.alpha =
      std::max(static_cast<tiny_ynn::float_t>(0.00001), optimizer.alpha);
