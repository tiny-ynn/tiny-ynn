...
TEST(convolutional, fprop) {
  ...
  l.setup(false);
  {
    ...
    for (auto o : out) EXPECT_DOUBLE_EQ(o, tiny_ynn::float_t(0.0));
  }
...
TEST(convolutional, with_dilation) {
  convolutional_layer l(7, 7, 3, 3, 1, 1, tiny_ynn::padding::valid, true, 1, 1,
                        2, 2);
  tensor_buf buf(l, false);
...
TEST(convolutional, fprop_avx) {
  ...
  l.set_backend_type(tiny_ynn::core::backend_t::internal);

  l.forward_propagation(buf.in_buf(), buf.out_buf());

  l.set_backend_type(tiny_ynn::core::backend_t::avx);
...
TEST(convolutional, fprop_nnp) {
  ...
  l.setup(false);
  {
    ...
    for (auto o : out) EXPECT_DOUBLE_EQ(o, tiny_ynn::float_t(0.5));
  }
