 ...
 * @return n samples
 */
std::vector<tiny_ynn::tensor_t> gen_dataset(const int n_samples,
                                            const int seq_len,
                                            const int batch_size) {
  // define and initialize input and output data.
  tiny_ynn::tensor_t input;
  tiny_ynn::tensor_t output;
  ...
  // fill return buffer
  std::vector<tiny_ynn::tensor_t> ret;
  ret.push_back(input);
...
void train(const int n_samples,
  ...
  std::vector<tiny_ynn::tensor_t> dataset =
  ...
  tiny_ynn::progress_display disp(n_samples);
  tiny_ynn::timer t;
  ...
  auto on_enumerate_epoch = [&]() {
    ...
    float loss = nn.get_loss<tiny_ynn::mse>(dataset[0], dataset[1]) / n_samples;
  ...
  while (true) {
    ...
    float num2 = 0;
    std::cin >> num2;
    tiny_ynn::vec_t out1 = nn.predict(tiny_ynn::vec_t({num1}));
    tiny_ynn::vec_t out2 = nn.predict(tiny_ynn::vec_t({num2}));
